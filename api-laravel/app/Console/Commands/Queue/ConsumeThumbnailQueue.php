...
class ConsumeThumbnailQueue extends Command
{
    protected $signature = 'queue:consume-s3-thumbnail-upload-events';

    protected $description = 'Command to process raw S3 thumbnail creation events';

    public function handle(SqsClient $sqsClient): int
    {
        $queueUrl = config('queue.connections.sqs.prefix') . '/' . config('queue.thumbnail_queue');
        ...
        try {
            ...
            foreach ($messages as $message) {
                ...
                if ($s3Info) {
                    ...
                    HandleThumbnailUpload::dispatch($jobData)->onQueue('default');
